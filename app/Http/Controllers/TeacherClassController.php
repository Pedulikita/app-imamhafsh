...
class TeacherClassController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if ($classes->isNotEmpty()) {
            $classIds = $classes->pluck('id');
            $totalExams = \App\Models\Exam::whereIn('teacher_class_id', $classIds)->count();
            $activeExams = \App\Models\Exam::whereIn('teacher_class_id', $classIds)->active()->count();
            $upcomingExams = \App\Models\Exam::whereIn('teacher_class_id', $classIds)->upcoming()->count();
        ...
        $scheduleStats = [];
        if ($user->hasRole('teacher')) {
            $todaySchedules = \App\Models\Schedule::whereHas('teacherClass', function($q) use ($user) {
            ...
            $weeklySchedules = \App\Models\Schedule::whereHas('teacherClass', function($q) use ($user) {
                                $q->where('teacher_id', $user->id);
                            })
                            ->currentWeek()
                            ->active()
            ...
            $scheduleStats = [
                ...
                'classes_this_week' => $weeklySchedules,
            ];
        }
