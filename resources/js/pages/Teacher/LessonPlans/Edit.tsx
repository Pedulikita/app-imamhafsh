...
export default function EditLessonPlan({ lessonPlan, teacherClasses, subjects }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        teacher_class_id: lessonPlan.teacher_class_id.toString(),
        ...
        materials_needed: lessonPlan.materials_needed || '',
        learning_activities: lessonPlan.learning_activities || [{ activity: '', duration: 30 }],
        assessment_methods: lessonPlan.assessment_methods || '',
        homework_assignment: lessonPlan.homework_assignment || '',
        notes: lessonPlan.notes || '',
        status: lessonPlan.status,
        reflection: lessonPlan.reflection || '',
        attendance_count: lessonPlan.attendance_count || '',
    });

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/teacher/lesson-plans/${lessonPlan.id}`);
    };
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Button variant="ghost" size="sm" onClick={() => router.visit(`/teacher/lesson-plans/${lessonPlan.id}`)}>
                            <ArrowLeft className="w-4 h-4" />
                        </Button>
                        <div>
                            <h1 className="text-3xl font-bold text-slate-900">Edit Lesson Plan</h1>
                            <p className="text-slate-600 mt-1">Update your lesson structure and activities</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Update the basic details of your lesson</CardDescription>
                                </CardHeader>
                            ...
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>Update the date and time for this lesson</CardDescription>
                                </CardHeader>
                                <CardContent className="space-y-4">
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    {/* Lesson Reflection - only show for completed lessons */}
                                    {(lessonPlan.status === 'completed' || data.status === 'completed') && (
                                        <div>
                                            <Label htmlFor="reflection">Lesson Reflection</Label>
                                            <Textarea
                                                id="reflection"
                                                value={data.reflection}
                                                onChange={(e) => setData('reflection', e.target.value)}
                                                placeholder="Reflect on how the lesson went..."
                                                rows={4}
                                            />
                                    ...
                                    {/* Attendance Count - only show for completed lessons */}
                                    {(lessonPlan.status === 'completed' || data.status === 'completed') && (
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                min="0"
                                            />
                                            {errors.attendance_count && <p className="text-sm text-red-600">{errors.attendance_count}</p>}
                                        </div>
                                    )}
                                </CardContent>
                            </Card>
                            ...
                            <Card>
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <Button type="submit" disabled={processing} className="w-full">
                                        ...
                                        {processing ? 'Updating...' : 'Update Lesson Plan'}
                                    </Button>
                                </CardContent>
                            </Card>
                        </div>
