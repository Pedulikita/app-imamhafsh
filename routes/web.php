...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    Route::prefix('admin')->name('admin.')->middleware(['auth', 'verified', 'role:super_admin'])->group(function () {
        ...
        Route::prefix('facilities/categories')->name('facilities.categories.')->group(function() {
            Route::get('/', [\App\Http\Controllers\Admin\FacilityCategoryController::class, 'index'])->name('index');
            Route::post('/', [\App\Http\Controllers\Admin\FacilityCategoryController::class, 'store'])->name('store');
            Route::put('/{category}', [\App\Http\Controllers\Admin\FacilityCategoryController::class, 'update'])->name('update');
            Route::delete('/{category}', [\App\Http\Controllers\Admin\FacilityCategoryController::class, 'destroy'])->name('destroy');
            Route::post('/reorder', [\App\Http\Controllers\Admin\FacilityCategoryController::class, 'reorder'])->name('reorder');
        });
        
        // PPDB Settings Management
        Route::prefix('ppdb')->name('ppdb.')->group(function() {
            Route::get('/settings', [\App\Http\Controllers\Admin\PPDBSettingController::class, 'index'])->name('settings');
            Route::post('/settings/{section}', [\App\Http\Controllers\Admin\PPDBSettingController::class, 'update'])->name('update');
            Route::post('/upload-image', [\App\Http\Controllers\Admin\PPDBSettingController::class, 'uploadImage'])->name('upload-image');
        });
        
        // Payment Management Routes (Admin/Super Admin only)
        Route::prefix('payments')->name('payments.')->group(function() {
            Route::get('/', [\App\Http\Controllers\Admin\PaymentController::class, 'index'])->name('index');
            Route::get('/students', [\App\Http\Controllers\Admin\PaymentController::class, 'students'])->name('students');
            Route::get('/fee-types', [\App\Http\Controllers\Admin\PaymentController::class, 'feeTypes'])->name('fee-types');
            ...
            // Fee type management
            Route::post('/fee-types', [\App\Http\Controllers\Admin\PaymentController::class, 'storeFeeType'])->name('fee-types.store');
            Route::put('/fee-types/{feeType}', [\App\Http\Controllers\Admin\PaymentController::class, 'updateFeeType'])->name('fee-types.update');
            Route::delete('/fee-types/{feeType}', [\App\Http\Controllers\Admin\PaymentController::class, 'destroyFeeType'])->name('fee-types.destroy');
            
            // Student payment management
            Route::post('/create-payment', [\App\Http\Controllers\Admin\PaymentController::class, 'createPayment'])->name('create-payment');
        });
    });
    ...
    Route::prefix('admin')->name('admin.')->middleware(['role:super_admin,editor'])->group(function () {
        ...
        Route::put('legal-content/privacy', [\App\Http\Controllers\Admin\LegalContentController::class, 'updatePrivacy'])->name('legal-content.privacy.update');
        Route::get('legal-content/terms', [\App\Http\Controllers\Admin\LegalContentController::class, 'editTerms'])->name('legal-content.terms.edit');
        Route::put('legal-content/terms', [\App\Http\Controllers\Admin\LegalContentController::class, 'updateTerms'])->name('legal-content.terms.update');
    ...
    Route::prefix('teacher')->name('teacher.')
        ...
        Route::prefix('lesson-plans')->name('lesson-plans.')->group(function() {
            ...
            Route::get('/calendar', [\App\Http\Controllers\Teacher\LessonPlanController::class, 'calendar'])->name('calendar');
            ...
            // Additional actions
            Route::post('/{lessonPlan}/duplicate', [\App\Http\Controllers\Teacher\LessonPlanController::class, 'duplicate'])->name('duplicate');
            Route::post('/{lessonPlan}/make-template', [\App\Http\Controllers\Teacher\LessonPlanController::class, 'makeTemplate'])->name('make-template');
        });
...
// Chat Routes - Real-time Communication
Route::middleware(['auth', 'verified'])->prefix('chat')->name('chat.')->group(function() {
    ...
    Route::post('/conversations', [\App\Http\Controllers\ChatController::class, 'createConversation'])->name('conversations.create');
    Route::get('/{conversation}', [\App\Http\Controllers\ChatController::class, 'show'])->name('show');
    Route::post('/{conversation}/message', [\App\Http\Controllers\ChatController::class, 'sendMessage'])->name('message.send');
    Route::post('/{conversation}/read', [\App\Http\Controllers\ChatController::class, 'markAsRead'])->name('mark.read');
});

// Users Route for Chat - Find users to start conversations with
    ...
    $users = \App\Models\User::where('id', '!=', $currentUser->id)
        ...
        ->map(function($user) use ($currentUser) {
            return [
                ...
                'can_chat_with' => ($currentUser->hasRole('teacher') && $user->hasRole('parent')) || 
                                  ($currentUser->hasRole('parent') && $user->hasRole('teacher'))
            ];
        })
        ->filter(fn($user) => $user['can_chat_with']);
    
    return \Inertia\Inertia::render('Chat/SelectUser', [
        'users' => $users,
        'currentUser' => $currentUser
    ]);
})->name('users.index');

// Behavior Reports Routes - Comprehensive Student Behavior Management
Route::middleware(['auth', 'verified'])->prefix('behavior-reports')->name('behavior-reports.')->group(function() {
    Route::get('/', [\App\Http\Controllers\BehaviorReportController::class, 'index'])->name('index');
    Route::get('/create', [\App\Http\Controllers\BehaviorReportController::class, 'create'])->name('create');
    Route::post('/', [\App\Http\Controllers\BehaviorReportController::class, 'store'])->name('store');
    Route::get('/analytics', [\App\Http\Controllers\BehaviorReportController::class, 'analytics'])->name('analytics');
    Route::get('/{behaviorReport}', [\App\Http\Controllers\BehaviorReportController::class, 'show'])->name('show');
    Route::get('/{behaviorReport}/edit', [\App\Http\Controllers\BehaviorReportController::class, 'edit'])->name('edit');
    Route::put('/{behaviorReport}', [\App\Http\Controllers\BehaviorReportController::class, 'update'])->name('update');
    Route::patch('/{behaviorReport}/status', [\App\Http\Controllers\BehaviorReportController::class, 'updateStatus'])->name('update-status');
    Route::post('/{behaviorReport}/submit', [\App\Http\Controllers\BehaviorReportController::class, 'submit'])->name('submit');
    Route::post('/{behaviorReport}/review', [\App\Http\Controllers\BehaviorReportController::class, 'review'])->name('review');
});
